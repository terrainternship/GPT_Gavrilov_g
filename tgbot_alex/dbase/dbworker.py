...
# TODO: SQLite не переделан до конца на asyncio
async def connect_db(db_type):
    """
...
async def create_db():
    ...
    #print(f'connect_db:\n - {POSTGRE_HOST =}\n - {POSTGRE_PORT =}\n - {POSTGRE_DB =}\n - {POSTGRE_USER =}\n - {POSTGRE_PASSW =}')
    try:
        async with await connect_db(DB_TYPE) as connection:
            async with connection.cursor() as cursor:
                ...
                if RECREATE_DB:
                    await cursor.execute('DROP TABLE IF EXISTS users')    # удалили старую таблицу
                ...
                if RECREATE_DB:
                    await cursor.execute('DROP TABLE IF EXISTS history')    # удалили старую таблицу
...
async def add_history(history_data):
    """
...
async def get_user_entry(user_id):
    """
...
async def update_last_interaction(user_id, last_interaction):
    """
...
async def update_last_dialog(user_id, last_dialog):
    """
...
async def update_last_chunks(user_id, last_chunks):
    """
...
async def update_qa(user_id, last_qa):
    """
...
async def update_dialog_score(user_id, dialog_score):
    """
...
async def update_dialog_state(user_id, dialog_state):
    """
...
async def update_last_num_token(user_id, num_token):
    """
...
async def update_num_queries(user_id, num_queries):
    """
...
async def update_last_time_duration(user_id, time_duration):
    """
...
async def get_dialog_state(user_id):
    """
    ...
    async with await connect_db(DB_TYPE) as connection:
        async with connection.cursor() as cursor:
            ...
            if DB_TYPE == 'SQLite3':
                await cursor.execute("SELECT dialog_state FROM users WHERE user_id = ?", (user_id,))
            elif DB_TYPE == 'POSTGRE':
                await cursor.execute("SELECT dialog_state FROM users WHERE user_id = %s", (user_id,))
            else:
                raise Exception("connect_db(): Не установленный тип БД")
                sys.exit(1)
...
async def get_num_queries(user_id):
    """
    ...
    async with await connect_db(DB_TYPE) as connection:
        async with connection.cursor() as cursor:
            ...
            if DB_TYPE == 'SQLite3':
                await cursor.execute("SELECT num_queries FROM users WHERE user_id = ?", (user_id,))
            elif DB_TYPE == 'POSTGRE':
                await cursor.execute("SELECT num_queries FROM users WHERE user_id = %s", (user_id,))
            else:
                raise Exception("connect_db(): Не установленный тип БД")
                sys.exit(1)
...
def get_df_users():
    ...
    """
    # Connect to the SQLite database
    connection = asyncio.run(connect_db(DB_TYPE))
