...
async def update_last_interaction(user_id: int, last_interaction: str) -> None:
    """
    Обновляет информацию о последнем взаимодействии пользователя.

    :param user_id: Идентификатор пользователя.
    :param last_interaction: Строка с информацией о последнем взаимодействии.
    """
    async with await connect_db() as session:
        stmt = update(User).where(User.user_id == user_id).values(last_interaction=last_interaction)
        await session.execute(stmt)
        await session.commit()

async def update_dialog_state(user_id: int, dialog_state: str) -> None:
    """
    ...
    """
    async with await connect_db() as session:
        stmt = update(User).where(User.user_id == user_id).values(dialog_state=dialog_state)
        await session.execute(stmt)
        await session.commit()

async def update_dialog_state_and_score(user_id: int, dialog_state: str, dialog_score: int) -> None:
    """
    ...
    """
    async with await connect_db() as session:
        stmt = update(User).where(User.user_id == user_id).values(dialog_state=dialog_state, dialog_score=dialog_score)
        await session.execute(stmt)
        await session.commit()

async def update_dialog_statistics(
        user_id: int, last_dialog: str, last_question: str,
...
async def get_dialog_state(user_id: int) -> Optional[str]:
    """
    ...
    """
    async with await connect_db() as session:
        result = await session.execute(select(User.dialog_state).where(User.user_id == user_id))
        return result.scalar_one_or_none()

async def get_num_queries(user_id: int) -> Optional[int]:
    """
    ...
    """
    async with await connect_db() as session:
        result = await session.execute(select(User.num_queries).where(User.user_id == user_id))
        return result.scalar_one_or_none()
